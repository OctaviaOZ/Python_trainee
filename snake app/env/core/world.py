...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            x = random.randrange(SNAKE_SIZE, SIZE[1] - SNAKE_SIZE)
            y = random.randrange(SNAKE_SIZE, SIZE[0] - SNAKE_SIZE)
            start_position = (x, y)
            # choose a random direction index
            start_direction_index = random.randrange(len(DIRECTIONS))
    ...
    def init_food(self):
        ...
        available_food_positions = set(zip(*np.where(self.get_observation() == 0)))
        if not self.custom:
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            if self.world[new_snake_head] == FOOD_BLOCK:
                # Remove old food
                self.world[new_snake_head] = 0
                # Add tail again
                self.snake.blocks += [old_snake_tail]
